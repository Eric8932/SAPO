...
from peft_custom import get_peft_config, get_peft_model, LoraConfig, TaskType, PeftModel, PeftConfig 
...
class DataTrainingArguments:
    ...
    max_num_instances_per_predict_task: int = field(
        default=1000, metadata={"help": "The maximum number of instances we will consider for each test task."}
    )
...
def main():
    ...
    base_path = full_path.parent.parent
    outputs_dir_to_create = base_path / "outputs"
    logs_dir_to_create = base_path / "logs"
    outputs_dir_to_create.mkdir(parents=True, exist_ok=True)
    logs_dir_to_create.mkdir(parents=True, exist_ok=True)
    ...
    if training_args.do_eval:
        ...
        logger.info(f"Total number of eval samples: {len(eval_dataset)}")
    ...
    if training_args.do_predict:
        ...
        logger.info(f"Total number of predict samples: {len(predict_dataset)}")

        num_gpus = training_args.world_size
        if num_gpus > 1:
            num_samples = len(predict_dataset)
            if num_samples % num_gpus != 0:
                new_size = (num_samples // num_gpus) * num_gpus
                predict_dataset = predict_dataset.select(range(new_size))
                logger.info(f"Final number of predict samples: {len(predict_dataset)}")
    ...
    # This block is for debugging the first training sample.
    if training_args.do_train:
        logger.info("="*80)

        processed_batch = data_collator([train_dataset[0],train_dataset[1]])

        input_ids = processed_batch['input_ids'][0]
        label_ids = processed_batch['labels'][0]
        logger.info(f"input_ids:\n{input_ids}")
        logger.info(f"attention:\n{processed_batch['attention_mask'][0]}")
        logger.info(f"inputs:\n{tokenizer.decode(input_ids, skip_special_tokens=False)}")
        logger.info(f"label_ids:\n{label_ids}")
        filtered_label_ids = [label_id for label_id in label_ids if label_id != -100]
        logger.info(f"labels:\n{tokenizer.decode(filtered_label_ids, skip_special_tokens=False)}")

        logger.info("="*80)

        input_ids = processed_batch['input_ids'][1]
        label_ids = processed_batch['labels'][1]
        logger.info(f"input_ids:\n{input_ids}")
        logger.info(f"attention:\n{processed_batch['attention_mask'][1]}")
        logger.info(f"inputs:\n{tokenizer.decode(input_ids, skip_special_tokens=False)}")
        logger.info(f"label_ids:\n{label_ids}")
        filtered_label_ids = [label_id for label_id in label_ids if label_id != -100]
        logger.info(f"labels:\n{tokenizer.decode(filtered_label_ids, skip_special_tokens=False)}")
    ...
    if training_args.do_predict:
        ...
        processed_batch = data_collator([predict_dataset[0],predict_dataset[1]])
    ...
    # Metric
    with open(data_args.suffix_file, 'r') as f:
    ...
    def compute_rouge_metrics(dataset, preds, save_prefix=None):
        ...
        decoded_preds = skip_instructions(preds, tokenizer, model_args.model_name_or_path, [suffixes[task]['suffix'] for task in dataset["Dataset"]])
        logger.info(f"\ndecoded_preds[0]:\n{decoded_preds[0]}")
    ...
    trainer._is_loss_eval_mode = False 
    ...
    if trainer.is_world_process_zero():
        convert_seqlora_adapter(os.path.join(training_args.output_dir, "adapter"))
