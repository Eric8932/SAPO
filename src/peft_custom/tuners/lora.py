...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def add_adapter(self, adapter_name, config=None):#from_pretrained
        ...
        mark_only_lora_as_trainable(self.model, self.peft_config[adapter_name].bias)#这个不重要，run_uie_lora里面又重置了
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if isinstance(target, LoraLayer):#已经是一个lora_layer了，那就加入r_sum。这里初始化没关系，后面会加载参数的感觉
                    target.update_layer(
                        adapter_name,
                        lora_config.r,
                        lora_config.lora_alpha,
                        lora_config.lora_dropout,
                        lora_config.init_lora_weights,#都是True
                    )
                else:
                    ...
                    self._replace_module(parent, target_name, new_module, target)#第一次加载adapter，替换原来的模块
        if not is_target_modules_in_base_model:
            raise ValueError(
...
class LoraLayer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, r_sum): # modified 
        ...
        if r > 0:
            ...
            self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r_sum, bias=False)})) # modified  如果是第一次，r_sum=0
    ...
    def update_layer_embedding(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):#不更新这里
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        if lora_dropout > 0.0:
            lora_dropout_layer = nn.Dropout(p=lora_dropout)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.dummy_A = nn.Parameter(torch.empty(r, self.in_features))
        # self.dummy_B = nn.Parameter(torch.empty(self.out_features, r))

        # 使用和您之前相同的初始化策略
        # nn.init.kaiming_uniform_(self.dummy_A, a=math.sqrt(5))
        # nn.init.zeros_(self.dummy_B)

    def merge(self):#这两个都没考虑
